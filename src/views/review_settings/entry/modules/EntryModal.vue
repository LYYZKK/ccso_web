    ...
    <a-spin :spinning="confirmLoading">
      <a-form :form="form">
        ...
        <a-form-item
          ...
          :wrapperCol="wrapperCol"
          label="评审需求">
          <a-textarea placeholder="请输入评审需求" v-decorator="['requirements', validatorRules.requirements]"/>
        </a-form-item>
        ...
        <a-form-item
          ...
          >
            <a-select-option v-for="(_type, key) in categoryData" :key="key" :value="_type.id">{{ _type.name }}
            </a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item
          ...
          >
            ...
            <a-select-option v-for="(_type, key) in informationData" :key="key" :value="_type.id">{{ _type.name }}
            </a-select-option>
          </a-select>
        </a-form-item>
        <a-form-item
  ...
  export default {
    ...
    methods: {
      async initSelectOptions() {
        let res = await getAction(this.url.informationUrl)
        this.informationData = res.success ? res.result : {}
        res = await getAction(this.url.categoryUrl)
        this.categoryData = res.success ? res.result : {}
      },
      add() {
        this.edit({});
      },
      edit(record) {
        ...
        this.informationValue = record.informationId
        this.categoryValue = record.categoryId
